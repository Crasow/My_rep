...
def list_extendleft(my_list):
    new_list = []
    new_list.insert(0, [my_list.pop() for el in range(len(my_list))])
    return new_list


def deque_extendleft(my_list):
    my_deque = deque([])
...
print(timeit(f'list_extendleft({a})', globals=globals(), number=100000))  # 4.763077
a = list_left()
print(timeit(f'deque_extendleft({a})', globals=globals(), number=100000))  # 0.6270194

"""
Безусловно список наполняется быстрее, но специфические функции
очередь выполняет в разы лучше.
"""